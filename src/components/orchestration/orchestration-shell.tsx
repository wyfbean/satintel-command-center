        ...
        {NAV.map(({ href, label, icon }) => {
          ...
          return (
            <Link key={href} href={href} title={label}
              className={`flex h-10 w-10 items-center justify-center rounded-xl transition ${active ? "bg-[#3d74ff] text-white" : "text-slate-400 hover:bg-white/10 hover:text-white"}`}>
              {icon}
            </Link>
          );
        })}
...
function AgentIcon() {
  return <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="8" r="4" /><path d="M6 20v-1a6 6 0 0 1 12 0v1" /><circle cx="18" cy="7" r="2" /><path d="M18 9v4" /></svg>;
}
function DashIcon() {
  return <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="3" width="18" height="18" rx="2" /><path d="M9 9h6M9 12h6M9 15h4" /></svg>;
}
function GlobeIcon() {
